...
var _clientId,     		// <-- GUID supplied by Azure
    ...
    _customAdalAuthUrl='%s?client_id=%s&response_type=%s&scope=%s&redirect_uri=%s',
...
function _getUserAuthorization(callback){
	...
	*/
	if (_clientId && _responseType && _scope && _redirectUrl) {
		...
		*/
		if (_customServer) {
			var url = String.format(_customAdalAuthUrl, _customAuthUrl, _clientId, _responseType, encodeURIComponent(_scope), encodeURIComponent(_redirectUrl));
		} else {
